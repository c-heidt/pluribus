...
LADDER_TOP_SECONDS=${LADDER_TOP_SECONDS:-750,750,60}          # MCCFR per-street (flop,turn,river) wall budget for the ladder TOP rung: top = probed it/s × this. Hard cells (flop/turn: hot_l1 0.23-0.43 at their production budgets) get the full 750s (raised from 600 for the new action grid, which widens the tree and pushes the convergence point deeper); river converged at ~9000 iters/22s so 60s brackets it without waste.
LADDER_TOP_SECONDS_VECTOR=${LADDER_TOP_SECONDS_VECTOR:-750,750,15}  # same for VECTOR cells — separate because the regimes converge at very different iteration counts (HU river vector settles ~1071 iters vs ~9000 multiway river MCCFR), so one per-street value cannot bracket both.
