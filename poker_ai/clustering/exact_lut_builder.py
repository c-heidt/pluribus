...
def _binary_search_combo(combos: np.ndarray, key: tuple) -> int:
    """
    Binary search for a combo in the sorted combos array.
    
    This is much more memory-efficient than using a dictionary index.
    The combos array is already sorted (generated by combinations() in lex order).
...
def _get_process_cache(street: str, save_dir: str):
    ...
    Uses combos array with binary search instead of dict index to save memory.
    ...
    if _PROCESS_CACHE[combos_key] is not None:
        return _PROCESS_CACHE[combos_key], _PROCESS_CACHE[memmap_key], _PROCESS_CACHE[centroids_key]
    ...
    with _CACHE_LOAD_LOCK:
        ...
        log.debug(f"Worker {os.getpid()} loading {street} combos array for binary search")
        ...
        if not centroids_path.exists():
            ...
            return combos, memmap, None
        
        centroids = np.load(centroids_path, allow_pickle=True)
        _PROCESS_CACHE[centroids_key] = centroids
        
        log.debug(f"Loaded {street} lookup data in process (combos: {len(combos)}, centroids: {len(centroids)})")
        return combos, memmap, centroids
...
class ExactHandStrengthBuilder(CardInfoLutBuilder):
    ...
    def compute_exact_turn_ehs_distribution(
        ...
        # Now loads combos array instead of dict index (saves ~560 MB per worker!)
        river_combos, river_memmap, river_centroids = _get_process_cache("river", self._config["save_dir"])
        ...
        for river_card in available_cards:
            ...
            # Lookup using binary search on sorted combos array
            ehs = None
            if river_combos is not None and river_memmap is not None:
                row_idx = _binary_search_combo(river_combos, key)
                if row_idx >= 0:
    ...
    def compute_exact_flop_distribution(
        ...
        # Now loads combos array instead of dict index (saves ~560 MB per worker!)
        turn_combos, turn_memmap, turn_centroids = _get_process_cache("turn", self._config["save_dir"])
        ...
        for turn_card in available_cards:
            ...
            if turn_combos is not None and turn_memmap is not None:
                row_idx = _binary_search_combo(turn_combos, key)
                if row_idx >= 0:
