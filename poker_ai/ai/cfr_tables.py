...
- Four :class:`~poker_ai.ai.index.InfosetIndex` instances — one per
...
- Four regret :class:`~poker_ai.ai.chunked_table.ChunkedTable`
  instances, one per street, exposed as ``tables.regret[r]``.
- Four strategy :class:`~poker_ai.ai.chunked_table.ChunkedTable`
...
class CFRTables:
    ...
    def __init__(
        ...
        actions_per_street : dict[int, int]
            ...
            actions.  Supplied by
            :data:`poker_ai.ai.action_space.MAX_ACTIONS_PER_STREET`.
        """
    ...
    def save_chunks(self, dir_path: Path) -> int:
        ...
        <poker_ai.ai.chunk_store.ChunkStore.save_dirty>` for each
    ...
    def validate_chunks(
        ...
        Used by :class:`~poker_ai.ai.checkpoint.CheckpointManager` to
    ...
    def restore_chunks(self, dir_path: Path, n_chunks: Dict[int, int]) -> None:
        ...
        <poker_ai.ai.chunk_store.ChunkStore.restore>`.  Missing
    ...
    def apply_discount(self, factor: float) -> None:
        ...
        :class:`~poker_ai.ai.training.DiscountState` schedule calls
    ...
    def reopen_after_fork(self) -> None:
        ...
        <poker_ai.ai.index.InfosetIndex.reopen_after_fork>` for
        details.
        """
