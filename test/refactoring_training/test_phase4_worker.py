...
class TestSyncToMaster:
    ...
    def test_flushes_single_infoset(self, tmp_path):
        ...
        w._sync_to_master()
        row = agent.regret_tables[0].get_row_if_exists("hand_A")
    ...
    def test_flushes_multiple_infosets(self, tmp_path):
        ...
        w._sync_to_master()
        assert agent.regret_tables[0].get_row_if_exists("hand_A") is not None
        assert agent.regret_tables[0].get_row_if_exists("hand_B") is not None

    def test_accumulates_onto_existing_regrets(self, tmp_path):
        """Sync adds to existing entries rather than replacing them."""
        w, agent = _make_worker(tmp_path)
        agent.regret_tables[0].merge_delta_row(
    ...
    def test_no_op_when_delta_is_empty(self, tmp_path):
        ...
        assert agent.regret_tables[0].n_allocated == 0
...
def _worker_target(job_queue, logging_queue, locks, agent, lut_path):
...
def test_terminate_flushes_before_exit(tmp_path):
    ...
    shm_dir = str(tmp_path / "shm")
    os.makedirs(shm_dir, exist_ok=True)
    agent = Agent(index_path=tmp_path / "lmdb_index", shm_dir=shm_dir)
    locks = {
        "strategy_update_lock": mp.Lock(),
    }
    job_queue = mp.JoinableQueue()
    ...
    proc = mp.Process(
        ...
        args=(job_queue, logging_queue, locks, agent, lut_path),
    )
    proc.start()
    ...
    total_allocated = sum(
        agent.regret_tables[r].n_allocated for r in range(4)
    )
