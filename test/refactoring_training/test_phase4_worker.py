...
Covers:
- 4.3  ``_flush_delta()`` flushes local delta into agent.regret
...
class TestSyncToMaster:
    ...
    def test_no_op_when_delta_is_empty(self, tmp_path):
        """_flush_delta() must return immediately when delta is empty."""
        w, tables = _make_worker(tmp_path)
        w._flush_delta()  # must not raise or block
...
def test_terminate_flushes_before_exit(tmp_path):
    """Terminate job must call _flush_delta() before the worker exits.
